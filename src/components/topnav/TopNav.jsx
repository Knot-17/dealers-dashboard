...
const handleLogout = () => {
  // delete table api
  axios
    .delete(`http://localhost:5000/api/v1/deleteAllrows`)
    .then((res) => {
      console.log(res);
    })
    .catch((err) => {
...
const TopNav = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:5000/api/v1/getDealer/${localStorage.getItem("id")}`
      )
      .then((res) => {
  ...
  localStorage.setItem("dealersID", dealer.dealersID);
